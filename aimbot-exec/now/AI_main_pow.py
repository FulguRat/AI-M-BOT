...
def capturing(array, the_class_name, the_hwnd_name, lock):
    ...
    win_cap = WindowCapture(the_class_name, the_hwnd_name, 4/9, 1.6)  # 初始化截图类
    ...
    while not arr[14]:
        ...
        if str(win32gui.GetForegroundWindow()) in (str(window_hwnd_name) + str(window_outer_hwnd)) and not test_win:  # 是否需要控制鼠标:
            ...
            if arr[6] and target_count and (arr[8] or GetAsyncKeyState(0x06) < 0):  # XButton2
                ...
                moveX = FOV(moveX, arr[5]) / DPI_Var[0] * arr[21] * (arr[0] / 512)
                moveY = FOV(moveY, arr[5]) / DPI_Var[0] * arr[21] * (arr[1] / 320)
